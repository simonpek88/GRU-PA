...
# 指定参数启动
streamlit run gru-pa.py --server.port 空闲端口 --server.address localhost或实际地址
```
...
screen -S gru-pa
streamlit run gru-pa.py --server.port 空闲端口 --server.address localhost或实际地址 --server.headless true
# 按Ctrl+A+D退出screen
...
# Windows后台运行
start /B python -m streamlit run gru-pa.py --server.port 8510 > gru-pa.log 2>&1
```
...
- 文件命名 : 站点_全站工作量统计_统计日期区间_随机编码.xlsx
- 工作表 :
  - Sheet1: 统计表
  - Sheet2: 简报
- 格式设置 :
...
- 复杂度要求 : 大小写字母+数字+特殊字符(建议)
- 长度要求 : 最少8位，推荐12位以上(建议)
- 更换周期 : 90天强制更换(Todo)
